...
addin_search_project_org <- function() {
  ...
  if(orgPath=="") { addin_error_org("Search Project Organisation") }
  ...
  runGadget(addin_search_org_ui(), addin_search_org_server,
            viewer = addin_create_dialog_viewer("Search Project Organisation", settings))

}



addin_search_org_ui <- function() {

  miniPage(

    #tags$style(HTML( get_css_theme() )),  # Apply the theme-specific CSS: dark or light
     # currently not implementing as the DT dataTableOutput is too dark!

    shinyjs::useShinyjs(), # to enable & disable inputs as needed
...
addin_search_org_server <- function(input, output, session) {
