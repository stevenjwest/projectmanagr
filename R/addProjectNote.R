...
addProjectNote <- function( projectNoteName, projectNotePrefix, projectNoteDir, selection,
  ...
  noteType <- list( "SINGLE" ) # noteType is single - no subNotes will be added
  attrs <- list(projectName, goalNum, delNum, taskNum, as.character(file.info(projNotePath)[,5]), noteType )
